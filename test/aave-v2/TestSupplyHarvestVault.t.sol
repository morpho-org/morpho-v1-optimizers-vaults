...
contract TestSupplyHarvestVault is TestSetupVaults {
    ...
    function testShouldWithdrawOnBehalfIfAllowed() public {
        ...
        vaultSupplier1.approve(address(ma2hDai), address(vaultSupplier2), shares);
    ...
    function testShouldClaimAndFoldRewards() public {
        ...
        assertEq(
            balanceOnPoolAfter,
            balanceOnPoolBefore + rewardsAmount.rayDiv(pool.getReserveNormalizedIncome(dai)),
            "unexpected balance on pool"
        );
        assertEq(
            ERC20(stkAave).balanceOf(address(daiSupplyHarvestVault)),
            0,
