...
contract SupplyHarvestVault is SupplyVaultBase, OwnableUpgradeable {
    ...
    /// @return rewardsAmount The amount of rewards claimed for each reward token (in underlying).
    /// @return rewardsFee The total amount of fees swapped and taken by the claimer (in underlying).
    function harvest() external returns (uint256 rewardsAmount, uint256 rewardsFee) {
        ...
        if (rewardsAmount > 0) {
            aave.safeTransfer(address(swapperMem), rewardsAmount);
            ...
            uint256 harvestingFeeMem = harvestingFee;
